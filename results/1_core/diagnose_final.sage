...
from estimator.lwe_dual import (
    matzov, early_abort_range, local_minimum, max_beta_global,
    red_cost_model_default, DH, exhaustive_search
)
...
def gj_cost_at_fixed_zeta(params, zeta_val):
    return DH.optimize_blocksize(
...
for name, (params, zetas) in targets.items():
    ...
    for z in zetas:
        try:
            ...
            m_log2 = f"ERR: {e}"
        try:
            g = gj_cost_at_fixed_zeta(params, z)
