...
def cost_fixed_zeta_true(k_enum_val, params, red_cost_model=red_cost_model_default, t_step=1):
    for p in early_abort_range(2, params.q):
        t_max = params.n - k_enum_val
        p_best = None
        for t in range(0, t_max + 1, t_step):
            precision = 1
            ...
            with local_minimum(40, max_beta, precision=precision) as it:
                for beta in it:
                    ...
                    it.update(cost)
            if p_best is None or it.y["rop"] < p_best["rop"]:
                p_best = it.y
        p[1].update(p_best)
        if p[1].y["t"] == 0 and p[0] > 2:
            break
...
# Module-level globals for multiprocessing workers (fork inherits these)
_WORKER_PARAMS = None
...
def _fine_worker(zeta):
    r = cost_fixed_zeta_true(zeta, _WORKER_PARAMS)
    log2rop = float(log(r["rop"], 2).n())
