...
def cost_fixed_zeta_warmstart(k_enum_val, params, t_hint=None, t_coarse_step=10, t_window=15,
    ...
    for p in early_abort_range(2, params.q):
        ...
        while True:
            ...
            window *= 2
            doublings += 1
        doublings_used = max(doublings_used, doublings)

        p[1].update(p_best)
        if p[1].y["t"] == 0 and p[0] > 2:
            break
    if verbose and doublings_used > 0:
        print(f"      [zeta={k_enum_val}] t-window doubled {doublings_used}x (final window={t_window * (2**doublings_used)})", flush=True)
    return p[1].y
...
def fine_scan_warmstart(params, center, window, t_seed, verbose=True):
    ...
    for zeta in range(center, hi + 1):
        zt0 = time.time()
        r = cost_fixed_zeta_warmstart(zeta, params, t_hint=t_hint, eval_counter=eval_counter, verbose=verbose)
        results[zeta] = r
        t_hint = int(r.get("t"))
        if verbose:
            log2rop = float(log(r["rop"], 2).n())
