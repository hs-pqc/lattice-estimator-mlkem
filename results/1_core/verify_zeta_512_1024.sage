...
def scan(name, params, step, zmax):
    params = params.normalize()
    print(f"=== {name} (MATZOV, step={step}) ===")
    print("zeta,log2_rop")
    results = []
    for zeta in range(0, zmax + 1, step):
        try:
    ...
    if results:
        ...
        print(f">>> {name} global minimum: zeta={best[0]}, log2(rop)={best[1]:.3f}")
    print()
    return results

MLKEM_512 = LWE.Parameters(n=512, q=3329, Xs=ND.CenteredBinomial(3), Xe=ND.CenteredBinomial(3), tag="ML-KEM-512")
MLKEM_1024 = LWE.Parameters(n=1024, q=3329, Xs=ND.CenteredBinomial(2), Xe=ND.CenteredBinomial(2), tag="ML-KEM-1024")

scan("ML-KEM-512", MLKEM_512, step=2, zmax=60)
scan("ML-KEM-1024", MLKEM_1024, step=2, zmax=60)

print("reference: LWE.dual_hybrid (MATZOV) reported zeta=0 for both ML-KEM-512 and ML-KEM-1024")
